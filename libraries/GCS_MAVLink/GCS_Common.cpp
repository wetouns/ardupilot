...
void GCS_MAVLINK::handle_radio_status(const mavlink_message_t &msg, bool log_radio)
{
    ...
    rssi->set_radio_remrssi(packet.remrssi/254.0f);
    rssi->set_radio_rssi(packet.rssi/254.0f);
    rssi->set_radio_noise(packet.noise);
    rssi->set_radio_remnoise(packet.remnoise);
    if (packet.remrssi != 0) {
